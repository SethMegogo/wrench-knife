...
*/
        ...
        try {
            ...
            System.out.println("Caught an exception  Recommender Films. Please re-run the tests");
        }

        try {
            CommonTests.booksTest(driver,wait,logger,loggerHTML,errorCounter,sub_url);
        } catch (InterruptedException e) {
            System.out.println("Caught an exception  Recommender Books. Please re-run the tests");
        }

        try {
            CommonTests.TVTest(driver,wait,logger,loggerHTML,errorCounter,sub_url);
        } catch (InterruptedException e) {
            System.out.println("Caught an exception  Recommender TV Shows. Please re-run the tests");
        }
