...
public class HelpfulMethods {
    ...
    public static PrintWriter startLogger(String weekNumber) throws FileNotFoundException, UnsupportedEncodingException {
        PrintWriter logger = new PrintWriter("report_week_"+weekNumber+".csv", "UTF-8");
        logger.println("Project, Bugs Opened, Bugs Closed, Features Opened, Features Closed, Total Bugs Open");
        return logger;
    }
    public static PrintWriter startLoggerHTML(String environment, String browser, String logName) throws FileNotFoundException, UnsupportedEncodingException {
        ...
        loggerHTML.println("Universe autotest v.0.2 Proudly Presents<br>");
        ...
        loggerHTML.println("<tr><th>Bug Description</th><th>Steps to Reproduce</th><th>Time</th><th>Actual Result</th><th>Expected Result</th></tr>");
    ...
    public static void finish(WebDriver driver, PrintWriter logger, PrintWriter loggerHTML, int errorCounter, String testName, String logName) throws IOException, InterruptedException {
        ...
        loggerHTML.println("<br>");
        loggerHTML.println("<font>26 test(s) performed</font><br>");
        loggerHTML.println("<font color=\"red\">"+errorCounter+" test(s) failed</font><br>");
        int testsPassed = 26 - errorCounter;
        loggerHTML.println("<font color=\"green\">"+testsPassed+" test(s) passed</font><br>");
        loggerHTML.println("Tests Completed with " + errorCounter + " error(s)<br>");
        loggerHTML.println("</BODY>");
        loggerHTML.println("</HTML>");
        loggerHTML.close();
